...
uint256 hashGenesisBlock("0xcb0031c3444bf64a1b4564da726bd26a56969a7329085c0203cfe6b26cd5515a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitcoinX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitcoinX Signed Message:\n";
...
{
    // BitcoinX: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // BitcoinX: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // BitcoinX: 1 days
static const int64 nTargetSpacing = 10 * 60; // BitcoinX: 10 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinX: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xac81aaa43aa8b79ca292ba230d4dc93e43ba6bfb753446e32973cc59762121c8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Dallas Morning News Immigration fight heats up 9/20/2019 11:11PM ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040a26340c39c748f014b4c49bb9a9faa08152603fd194e3900bf40914d6dbdb29320c48d7b0bd892b305a789d12683307ebfdfcb8e3167056ef2d09da9c001e6f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1569039556;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084947397;

        if (fTestNet)
        {
            block.nTime    = 1569039535;
            block.nNonce   = 387506947;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xdfcee10d5a84f654927da35d2aabd2a00763aa94beeaaaa0ec712311896b5788"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xce, 0xb9, 0xda }; // BitcoinX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinXMiner(CWallet *pwallet)
{
    printf("BitcoinXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BitcoinX-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinXMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinXMiner, pwallet));
}
